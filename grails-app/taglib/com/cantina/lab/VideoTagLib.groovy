...
/**
 * A tag library that displays video assets for Grails.
 *
...
class VideoTagLib {
    ...
    def TYPE_FLOWPLAYER = "flowplayer"
    def TYPE_JWFLV = "jwflv"
    ...
    def includes = {    attrs ->
        ...
        if (pluginContextPath?.length() > 0 && pluginContextPath != '/') {
            prefix = prefix + pluginContextPath
        }
        out << """\
    ...
    def display = { attrs ->
        ...
        if (player == TYPE_JWFLV) {
            ...
            def playerId = "player" + player + movie.id + stream
